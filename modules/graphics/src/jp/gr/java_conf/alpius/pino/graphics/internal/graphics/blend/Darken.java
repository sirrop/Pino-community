...
public class Darken implements SimpleBlender.BlendStrategy {
    private interface SingletonHolder {
        Darken instance = new Darken();
    }
    ...
    public int calc(int src, int dst) {
        ...
        if (srcAlpha == 255 && dstAlpha == 255) {
            return 255 << 24 |
                    Math.min(srcRed, dstRed) << 16 |
                    Math.min(srcGreen, dstGreen) << 8 |
                    Math.min(srcBlue, dstBlue);
        }
